      ...
      value: function() {
        ...
        var failFunction = function(reason) {
          ...
          if (LocalFieldDB && LocalFieldDB.FieldDBObject && typeof LocalFieldDB.FieldDBObject.bug === "function") {
            LocalFieldDB.FieldDBObject.bug("There was a problem opening your data. " + reason.userFriendlyErrors);
          }
        };
        ...
        Q.allSettled(promises).then(function(results) {
          results.map(function(result) {
            if (result.state === "fulfilled" && result.value && result.value.ok) {
              ...
              FieldDB.FieldDBObject.bug("One of your changes was not saved " + cleanedDatum._id + " " + result.value.userFriendlyErrors.join("\n"));
            }
          });
          ...
          var cleanAndSaveIfChanged = function(e) {
            var result = e.target.parentElement.__data__.clean().then(function(proposedChanges) {
              if (proposedChanges.length > 0) {
                ...
                var saveEditToAllData = FieldDB.FieldDBObject.confirm("Would you like to clean this lexical entry? (This will change all examples you see here to have this new information.)\n\n" + changesAsStrings.join("\n"));
                if (saveEditToAllData) {
                  e.target.parentElement.__data__.save().then(function(result) {
                    console.log("Saving success...", result);
                    FieldDB.FieldDBObject.popup("Saved " + changesAsStrings.join(" "));
                  }, function(reason) {
                    console.log("Saving failed...", reason);
                    FieldDB.FieldDBObject.bug("Save failed. " + reason.userFriendlyErrors.join(" "));
                  }).fail(function(reason) {
                    console.log("Saving failed...", reason);
                    FieldDB.FieldDBObject.bug("Save failed. Please notify the app's developers " + e.target.parentElement.__data__.datumids.join(" "));
                  });
                }
              }
            });
