(function(exports) {
  ...
  var LexiconNode = function(options) {
    ...
    this.fossil = JSON.parse(JSON.stringify(options.fields));
  ...
  LexiconNode.prototype = Object.create(BASE_LEXICON_NODE.prototype, {
    ...
    equals: {
      value: function(b) {
        ...
        for (fieldIndex in this.expandedIGTFields) {
          ...
          if (a.fields.hasOwnProperty(field)) {
            // console.log(a);
    ...
    compare: {
      value: function(b) {
        var a = this;
        var result = 0;
        if (!b || !b.fields || !b.fields[this.sortBy]) {
          return -1;
        }
        if (!a || !a.fields || !a.fields[this.sortBy]) {
          return 1;
        }
        if ((typeof(a.fields[this.sortBy]) === 'number') && (typeof(b.fields[this.sortBy]) === 'number')) {
          result = a.fields[this.sortBy] - b.fields[this.sortBy];
        } else if ((typeof(a.fields[this.sortBy]) === 'string') && (typeof(b.fields[this.sortBy]) === 'string')) {
          if (a.fields[this.sortBy] < b.fields[this.sortBy]) {
            result = -1;
          } else if (a.fields[this.sortBy] > b.fields[this.sortBy]) {
            result = 1;
          } else {
            result = 0;
          }
        } else if (typeof(a.fields[this.sortBy]) === 'string') {
          result = 1;
        } else {
          result = -1;
        }
      ...
      value: function() {
        ...
        var successFunction = function(doc) {
          ...
          doc.datumFields.map(function(datumField) {
            if (self.fossil[datumField.label] !== self.fields[datumField.label] && (new RegExp(escapeRegexCharacters(self.fossil[datumField.label]), "i")).test(datumField.mask)) {
              var change = {
                before: datumField.mask + ""
              };
              // TODO this makes things lower case... because part of the map reduce seems to be doing that...
              datumField.mask = datumField.mask.replace(new RegExp(escapeRegexCharacters(self.fossil[datumField.label]), "ig"), self.fields[datumField.label]);
        ...
        if (!lexicalEntryToMatch) {
          ...
          this.filter(function(value, key, object, depth) {
            ...
            if (typeof lexicalEntryToMatch.equals === "function") {
              ...
              for (var attr in value.fields) {
                if (value.fields.hasOwnProperty(attr) && value.fields[attr] === lexicalEntryToMatch) {
    ...
    bindToView: {
      value: function() {
        ...
        this.forEach(function(entry) {
          ...
          if (entry.fields && entry.fields.morphemes === "@") {
            return;
          }
          ...
          var toggleEditMode = function(e) {
            ...
            if (e.target.jsonView.hidden) {
              ...
              try {
                for (var field in newIgt) {
                  if (newIgt.hasOwnProperty(field)) {
                    // e.target.__data__.fields[field] = newIgt[field];
                  }
                }
              } catch (except) {
              ...
              e.target.jsonView.innerHTML = JSON.stringify(e.target.__data__.fields, null, 2);
            }
          ...
          listItemView.style.opacity = listItemView.__data__.fields.confidence;
          listItemView.classList.add("lexical-entry");
          listItemView.classList.add("scrollable");
          if (listItemView.__data__.fields.morphemes) {
            listItemView.id = listItemView.__data__.fields.morphemes;
          }
          ...
          listItemView.__data__.fields.headword = listItemView.__data__.fields.headword || listItemView.__data__.fields.morphemes ? listItemView.__data__.fields.morphemes : listItemView.__data__.fields.gloss;
          ...
          for (field in listItemView.__data__.fields) {
            if (listItemView.__data__.fields.hasOwnProperty(field)) {
              if (field === "discussion" || field === "headword") {
                continue;
              }
              try {
                headword.classList.add(field + ":" + listItemView.__data__.fields[field]);
                discussion.classList.add(field + ":" + listItemView.__data__.fields[field]);
                ...
                fieldLabelElement.classList.add(listItemView.__data__.fields[field]);
                ...
                fieldElement.classList.add(listItemView.__data__.fields[field]);
                ...
                var dataPath = "listItemView.__data__.fields." + field;
                var bindSet = {};
                bindSet[viewPath] = {
                  "<-": dataPath
                };
          ...
          Bindings.defineBindings(component, {
            "headwordView.value": {
              "<-": "listItemView.__data__.fields.headword"
            },
            "saveButtonView.innerHTML": {
              "<-": "'Save '+listItemView.__data__.fields.headword"
            },
            // "saveButtonView.classList.has('btn-danger')": {
            //   "<-": "listItemView.__data__.unsaved"
            // },
            "discussionView.value": {
              "<-": "listItemView.__data__.fields.discussion"
            },
            ...
            "listItemView.hidden": {
              "<-": "listItemView.__data__.fields.confidence < self.localDOM.getElementById('lexiconConfidenceThreshold').value / 10"
            }
          });
    ...
    guessContextSensitiveGlosses: {
      value: function(datum) {
        ...
        var matchingfunction = function(node) {
          if (node.fields.morphemes === morphemeToFind) {
            // console.log(node);
        ...
        for (var group in morphemeGroup) {
          ...
          for (var m in morphemes) {
            ...
            if (matchingNodes && matchingNodes.length > 0) {
              ...
              try {
                gloss = matchingNodes[0].fields.gloss;
              } catch (e) {
    ...
    if (options.precedenceRelations && options.precedenceRelations.length > 0) {
      for (var i in options.precedenceRelations) {
        try {
          ...
          // }));
          lex.add(new LexiconNode({
            fields: options.precedenceRelations[i].key.subsequent,
            datumids: [options.precedenceRelations[i].key.context.id],
            utteranceContext: [options.precedenceRelations[i].key.context.utterance],
            url: options.url
          }));
